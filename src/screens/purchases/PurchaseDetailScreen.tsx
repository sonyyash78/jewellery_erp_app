...
export default function PurchaseDetailScreen({ route, navigation }: any) {
  ...
  const handleCancel = () => {
    Alert.alert('Cancel Purchase', 'Are you sure you want to cancel this purchase? This will mark it as Cancelled.', [
  ...
  const handlePreview = async () => {
    try {
      ...
      console.error(err);
      Alert.alert('Error', 'Failed to open purchase preview');
    } finally {
      setProcessing(false);
    }
  };
  ...
  const handleShare = async () => {
    try {
      ...
      const pdfName = `Purchase_${(pdfData.invoice?.invoice_number || purchaseId).toString().replace(/[^a-zA-Z0-9-]/g, '_')}.pdf`;
      ...
      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(newUri, { UTI: '.pdf', mimeType: 'application/pdf', dialogTitle: 'Share Purchase Receipt' });
      } else {
        Alert.alert('Success', `Purchase receipt saved at: ${newUri}`);
      }
    } catch (err) {
      console.error(err);
    ...
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.headerCard}>
          <Text style={styles.title}>{invoice.invoice_number || `PUR-${purchaseId}`}</Text>
          <Text style={styles.date}>{new Date(invoice.invoice_date || Date.now()).toLocaleString('en-IN')}</Text>
          <View style={styles.statusBadge}>
            <Text style={styles.statusText}>{invoice.status || 'Completed'}</Text>
          </View>
        </View>
        ...
        {processing && (
          <View style={styles.processingBanner}>
            ...
            <Text style={styles.processingText}>Preparing bill preview/PDF...</Text>
          </View>
        )}

        <View style={styles.card}>
          <Text style={styles.sectionTitle}>Supplier Details</Text>
          <Text style={styles.customerName}>{pdfData.customer?.name || 'Walk-in Supplier'}</Text>
        ...
        <View style={styles.card}>
          ...
          {items.map((item: any, idx: number) => (
            <View key={idx} style={styles.itemRow}>
              ...
              <Text style={styles.itemPrice}>₹ {fmt(item.final_price || item.metal_value)}</Text>
            </View>
          ))}
        </View>
        ...
        <View style={styles.settlementCard}>
          ...
          <View style={styles.detailRow}>
            ...
            <Text style={[styles.detailValue, { color: '#4ade80' }]}>₹ {fmt(invoice.cash_paid || invoice.amount_paid)}</Text>
          </View>
