...
export default function CreateCRMScreen({ route, navigation }: any) {
  ...
  const [formData, setFormData] = useState({
    ...
    mobile: item ? (item.phone_number || item.mobile || '').replace(/^91/, '') : '',
    ...
    try {
      ...
      if (type === 'Supplier') {
        const payload = {
          ...
          aadhaar_pan: formData.aadhaar_pan.trim() || undefined,
      ...
      const detail = error.response?.data?.detail || `Failed to save ${type}. Please check your inputs.`;
