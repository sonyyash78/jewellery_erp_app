...
export default function EditInventoryScreen({ route, navigation }: any) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log('Failed to update item', error.response?.data || error.message);
      ...
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        ...
        <View style={styles.actionRow}>
          ...
          <TouchableOpacity style={styles.submitButton} onPress={handleSubmit} disabled={loading}>
            {loading ? <ActivityIndicator color="#0a0a0a" /> : <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}><Ionicons name="qr-code-outline" size={16} color="#0a0a0a" /><Text style={styles.submitButtonText}>Save Changes</Text></View>}
          </TouchableOpacity>
        </View>
